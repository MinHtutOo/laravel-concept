<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;

class PostsController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        return "I am index method of posts controller";
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(string $id)
    {
        return "Showing id " . $id;
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(string $id)
    {
        return "Editing id ". $id;
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, string $id)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(string $id)
    {
        //
    }

    // public function contact($username,$password)
    // {
    //     return "The owner of this post is <b>$username</b> and password is <b>$password</b>";
    // }

    public function home($name,$country)
    {
        // return view("home")->with(array('name'=> $name,'country'=> $country));
        return view("home",compact("name","country"));
    }

    public function about()
    {
        $persons = ["Bruce","Alex","Roxy","Oliver","Bryan"];
        return view("about",compact("persons"));
    }

    public function contact()
    {
        return view("contact");
    }
}
